...
import com.fiap.techchallenge2.service.impl.EstacionamentoServiceImpl;
...
class EstacionamentoIniciaTests {
	...
	public void deveRetornarStatus201() throws Exception {
		...
		Assertions.assertEquals(dataMock.plusMinutes(30), responseApp.horarioDeSaida());

		mockData.close();
	}

	@Test
	public void deveRetornarStatus201_anotherDay() throws Exception {
		var mockData = Mockito.mockStatic(LocalDateTime.class, Mockito.CALLS_REAL_METHODS);
		...
		Assertions.assertEquals("ABC1A23", responseApp.placa());
		Assertions.assertEquals(dataMock, responseApp.horarioDeEntrada());
		Assertions.assertEquals(dataMock.plusMinutes(270), responseApp.horarioDeSaida());
	...
	private static Stream<Arguments> requestValidas() {
		return Stream.of(
				Arguments.of("ABC1A23", TempoPermanenciaEnum.MEIA_HORA),
				Arguments.of("ABC1A23", TempoPermanenciaEnum.UMA_HORA),
				Arguments.of("ABC1A23", TempoPermanenciaEnum.UMA_HORA_E_MEIA)
		);
	}
